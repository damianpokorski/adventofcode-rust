...
fn part1() -> (i32, i32) {
  ...
  let disallowed_strings: Vec<String> = (vec!["ab", "cd", "pq", "xy"]).into_iter().map(|x| String::from(x)).collect();
  let empty_letter = "_";

  // Iterate through lines
  for line in read_file().split_ascii_whitespace().map(|x| String::from(x)).into_iter() {
...
fn part2() -> (i32, i32) {
  ...
  for line in read_file().split_ascii_whitespace().map(|x| String::from(x)).into_iter() {
    ...
    let single_letter_repeat_with_gap = Regex::new(r"([a-z]{1}).\1").unwrap().is_match(&line).unwrap();
    let two_letter_reapting =  Regex::new(r"([a-z]{2}).*?\1").unwrap().is_match(&line).unwrap();
